    ...
    properties: [
      {
        parameters: [
          ...
          {
            name: 'generateTitle',
            type: 'boolean | { model: DynamicArgument<MastraLanguageModel>; instructions?: DynamicArgument<string>; awaitGeneration?: boolean }',
            description:
              'Controls automatic thread title generation from the conversation transcript. Can be a boolean or an object with custom model and instructions. By default title generation runs in the background after the response is returned; set `awaitGeneration: true` to wait for it to finish (and persist) before `generate()`/`stream()` resolves. This matters on runtimes that can freeze the execution environment right after the response is sent (e.g. AWS Lambda, Vercel Functions), where a background title generation may otherwise never complete.',
            isOptional: true,
            defaultValue: 'false',
          },
        ],
